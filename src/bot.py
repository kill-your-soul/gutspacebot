...
async def time(m: Message) -> None:
    ...
    if len(btime) == 0:
        ...
        keyboard.add(Text("Задать вопрос"), color=KeyboardButtonColor.PRIMARY)
        await m.answer(
            "Команда коворкинга работает с 9 до 21 часа и готова ответить на все ваши вопросы только в эти часы. В остальное время мы отдыхаем и вам советуем💙",
            keyboard=keyboard,
        )
